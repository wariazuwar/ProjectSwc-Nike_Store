<?php
   ...
   //CHECK IF THE USER IS LOGGED IN
    ...
    $sql_checkOrder = "SELECT * FROM cart WHERE cart_id='$cartID' AND status='paid'";
    $res_checkOrder = mysqli_query($conn, $sql_checkOrder);

    if(mysqli_num_rows($res_checkOrder) > 0){
        header("location: toship.php");
    }
    ...
    if(isset($_POST['submit'])){
        ...
        $totalAddress = "".$address.", ".$postCode.", ".$city.", ".$state.", ".$country."";
        
        //CHECK FOR SECURITY REASON
        $securityCheck = "SELECT * FROM cart WHERE cart_id='$cartID' AND uid='$uid'";
        $res_securityCheck = mysqli_query($conn, $securityCheck);

        if(mysqli_num_rows($res_securityCheck) > 0){
            $Date = date("Y-m-d");
            $sql_update = "UPDATE cart SET status='paid', address='$totalAddress', date='$Date' WHERE cart_id='$cartID' AND uid='$uid'";
            ...
            if(mysqli_query($conn, $sql_update)){
                ...
                echo "Error! Please check database with ".$sql_updatep."";
            }
        }
    }
...
<html>
   ...
   <body>
        ...
        <section class="homepage-section">
            <div class='paynow-container'>
                ...
                <div class="address-container-flex">
                    <div class="address-contianer-left">
                        <label style="color: black;" for="post-code">Post Code:</label>
                        <input name="post_code" type="text" required></input>
                        <br><br>
                        <label style="color: black;" for="post-code">State:</label>
                        <input name="state" type="text" required></input>
                    </div>

                    <div class="address-container-right">
                        <label style="color: black;" for="post-code">City:</label>
                        <input name="city" type="text" required></input>
                        <br><br>
                        <label style="color: black;" for="post-code">Country:</label>
                        <input name="country" type="text" required></input>
                    </div>
                </div>
                ...
                <div onclick="showCard();" id="check-out-btn">
                    <span>Checkout</span>
                </div>
            <div>
        </section>

        <section style="display: none;" id="credit-cart-hidden">
            <div style="width: 100%; margin: auto;">
                ...
                <div class="card-flex-box">
                    ...
                    <div class="field-container">
                        ...
                        <input required id="expirationdate" type="text"  inputmode="numeric">
                    </div>
                    <div class="field-container">
                        <label for="securitycode">Security Code</label>
                        <input required id="securitycode" type="text"  inputmode="numeric">
                    </div>
                    </div>
                </div>
            </div>
            <br><br>
            <center>
                <input type="submit" value="Pay Now" name="submit" id="pay-now-btn"></input>
            </center>
            <div onclick="showCard();" id="close-btn"><span>x</span></div>
        </section>
        </form>
        ...
        <script>
            function showCard() {
                var x = document.getElementById("credit-cart-hidden");
