<?php
   ...
   //CHECK IF THE USER IS LOGGED IN
    ...
    if(isset($_POST['pay'])){
        ...
        header("location: mycart_paynow.php?cartID=".$cartID."");
    }
    ...
    if(isset($_POST['save'])){
        ...
        $sql_updateCart = "UPDATE cart SET amount='$amount' WHERE cart_id='$cartID'";
        $res_updateCart = mysqli_query($conn, $sql_updateCart);
    }

    if(isset($_POST['delete'])){
        $cartID = $_POST['cart_id'];

        $sql_deleteCart = "DELETE FROM cart WHERE cart_id='$cartID'";
        $res_deleteCart = mysqli_query($conn, $sql_deleteCart);
    }
        ...
        <section class="homepage-section">
            <div class="cart-nav-bar">
                <div onclick="window.location='mycart.php'" class="cart-nav-bar-items">
                    <span style="border-bottom: 2px solid black;">To Pay</span>
                </div>
            ...
            <div class="cart-container">
                <?php
                    $sql_checkCart = "SELECT * FROM cart WHERE uid='$uid' AND status='unpaid'";
                        ...
                        while($row = mysqli_fetch_array($res_checkCart)){
                            ...
                            echo "<div class=\"cart-container-flex-box\">";
                                echo "<div class=\"cart-container-flex-box-left\">";
                                    echo "<div class=\"cart-container-flex-box-left-up\">";
                                        ...
                                        echo "<div class=\"cart-container-flex-box-left-title\">";
                                            echo "<span id='mycart-product-title'>".$row_productInformation['name']."</span>";
                                            echo "<br><br>";
                                            echo "<span>Amount:</span>";
                                            echo "<br>";
                                            echo "<input style='width: 100px;' type='number' min='1' value='".$row['amount']."' name='amount'></input>";
                                        echo "</div>";
                                    echo "</div>";
                                ...
                                echo "<div class=\"cart-container-flex-box-right\">";
                                    echo "<input type='submit' value='Pay Now' id='mycart-btn' name='pay'></input>";

                                    echo "<input type='submit' value='Save' id='mycart-btn' name='save'></input>";

                                    echo "<input type='submit' value='Delete' id='mycart-btn' name='delete'></input>";
                                echo "</div>";
                            echo "</div>";
